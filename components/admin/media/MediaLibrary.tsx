...
function MetadataForm({asset,onSaved}:{asset:MediaAsset;onSaved:(asset:MediaAsset)=>void}){
  ...
  async function save(event:FormEvent<HTMLFormElement>){event.preventDefault();const fd=new FormData(event.currentTarget);setBusy(true);setMessage('');const body={id:asset.id,altText:String(fd.get('altText')??''),caption:String(fd.get('caption')??''),tags:String(fd.get('tags')??'').split(',').map(x=>x.trim()).filter(Boolean),sourceType:String(fd.get('sourceType')??'original') as ImageSourceType,rightsNote:String(fd.get('rightsNote')??''),focalX:fd.get('focalX')===''?null:Number(fd.get('focalX')),focalY:fd.get('focalY')===''?null:Number(fd.get('focalY')),visualQaApproved:fd.get('visualQaApproved')==='on'};const response=await fetch('/api/media',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify(body)});const result=await response.json();if(response.ok){onSaved(result.item);setMessage('Metadata saved.')}else setMessage(result.error??'Could not save metadata.');setBusy(false)}
  const blocked=getImagePublishBlockReason(asset)
