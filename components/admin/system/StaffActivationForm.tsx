...
export function StaffActivationForm({ token }: { token: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (passwordError) throw new Error('Could not set your password. Open the invitation link again and retry.')
      const response = await fetch('/api/auth/staff/accept', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ token }),
      })
      const body = await response.json()
      if (!response.ok) throw new Error(body.error ?? 'Officer activation failed.')
      window.location.assign('/admin')
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'Officer activation failed.')
    } finally {
      setBusy(false)
    }
  }
