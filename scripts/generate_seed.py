#!/usr/bin/env python3
"""Generate initial Supabase seed SQL from the versioned JSON fallbacks."""
...
TABLES: dict[str, tuple[str, list[str], set[str]]] = {
    ...
    "leaders": ("leaders.json", ["id","name","role","term","class_year","major","bio","photo_url","linkedin_url","email","current","advisor","open_seat","published","sort_order"], set()),
    "events": ("events.json", ["id","slug","title","summary","description","event_type","date_label","start_at","end_at","location","registration_url","cover_url","featured","published"], {"start_at","end_at"}),
    "resources": ("resources.json", ["id","title","description","category","source","url","pinned","published","sort_order"], set()),
    ...
    "partner_schools": ("partners.json", ["id","name","short_name","location","region_code","url","questions","published","sort_order"], set()),
...
def quote(value, nullable: bool = False) -> str:
...
def upsert(table: str, rows: list[dict], columns: list[str], nullable: set[str]) -> str:
    values = []
    for row in rows:
        values.append("(" + ", ".join(quote(row.get(column), column in nullable) for column in columns) + ")")
...
def main() -> None:
    ...
    blocks.append(
        "insert into public.site_settings (id, settings, published) values "
        f"('main', {quote(site)}, true) on conflict (id) do update set settings = excluded.settings, published = true;\n"
    )
    for table, (filename, columns, nullable) in TABLES.items():
        data = json.loads((CONTENT / filename).read_text(encoding="utf-8"))
        if table == "impact":
            rows = [{"id": "main", **data, "published": True}]
        else:
            rows = data if isinstance(data, list) else [data]
        if rows:
            blocks.append(upsert(table, rows, columns, nullable))
